...
name: onedz_zh
description:
  "分析 OneDZ 全球碎屑锆石数据库的 U-Pb 和 Lu-Hf 同位素数据。
  当用户询问锆石地质年代学、碎屑锆石分析、U-Pb 定年、Lu-Hf 同位素、
  年龄分布图（KDE/PDP）、谐和图、εHf(t) 演化或锆石年龄的地质统计分析时使用。
  也触发关于锆石数据过滤、概率密度图、年龄分布峰值检测或
  将地球化学数据导出为 GeoJSON/Shapefile 格式的查询。"
version: 1.2.0
language: zh
---
...
详细安装说明请参阅 [references/environment.md](references/environment.md)。
...
**默认数据路径**（`scripts/onedz_handler/config.py`）指向 `modified/` 子目录，包含格式修正后的数据。如该路径不存在，请参照下方说明操作。

### 数据集路径配置（Claude 使用说明）

> **重要**: 首次使用此 skill 时，必须确认数据集存在于默认路径。

当 skill 被调用但默认路径找不到数据集时：

1. **读取** `scripts/onedz_handler/config.py` 查看当前 `_DEFAULT_DATA_DIR` 的值
2. **询问用户** 提供正确的数据集目录路径（如："请提供 OneDZ 数据集 (zircon_upb.csv, zircon_luhf.csv) 所在的目录路径"）
3. **更新** `scripts/onedz_handler/config.py`：将 `_DEFAULT_DATA_DIR` 改为用户提供的路径
4. **重试** 任务

数据集目录必须至少包含：
- `zircon_upb.csv`（U-Pb 年龄数据）
- `zircon_luhf.csv`（Lu-Hf 同位素数据）
